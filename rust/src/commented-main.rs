...
fn decrypt_solver_thread(lower: u32,
                         ...
                         crypts: Arc<Vec<u64>>)
                         -> thread::JoinHandle<()> {
    // thread::spawn returns immediately
    thread::spawn(move || { // closure/inline function begins here
        for i in iter::range_inclusive(lower, upper) {
            // rewrite unsigned key to an array
            let key = unsafe { mem::transmute::<u32, [u8; 4]>(i) };
            let maybe_key = {
                map.get(&decrypt_block(crypts[0], &key, 4))
            };
            if let Some(lower_key) = maybe_key {
                // make array from unsigned
                let lower_key = unsafe { mem::transmute::<u32, [u8; 4]>(*lower_key) };
                let key = lower_key.iter()
                                    // lower key is only three bytes, so only read those
                                   .take(3)
                                   // when lower key ends, continue with the rest of the key
                                   .chain(key.iter())
                                   // make value from reference
                                   .map(|&x| x)
                                   // collect entire key to a vector
                                   .collect::<Vec<u8>>();
                // create pairs of (plain, crypt) for all messages, check that
                // plain encrypted with our key equals crypt for every pair
                if plains.iter()
                         .zip(crypts.iter())
                         .all(|(&p, &c)| encrypt_block(p, &*key, 6) == c) {
                    // we found the key! print it and exit!
...
fn main() {
    ...
    // reference counted, can share between threads.
    let shared_plains = Arc::new(plains);
    let shared_crypts = Arc::new(crypts);
    let mut hashmap: FnvHashMap<u64, u32> = Default::default();
    // 1<<24 = 0xFFFFFF = 3*8 = 3 Bytes
    hashmap.reserve(1 << 24);
    for i in 0..1 << 24 {
        hashmap.insert(encrypt_block(shared_plains[0],
                                     &unsafe { mem::transmute::<u32, [u8; 4]>(i) }, // cast u32 to array. 0xFFFFFF becomes [0xFF, 0xFF, 0xFF]
                                     3), // three rounds of encryption algorithm
                       i);
    }
    // make hashmap reference counted, shareable between threads.
    ...
    let join_handles: Vec<_> = iter::range_inclusive(0, 7)
                                    // create a new thread
                                   .map(|i| {
                                       //every thread gets one eighth of 0xFFFFFF (4 bytes)
                                       decrypt_solver_thread(i * (0xFFFFFFFF / 8), // lower boundary
                                                             (i + 1) * (0xFFFFFFFF / 8), //upper boundary
                                                             shared_map.clone(),
                                                             shared_plains.clone(),
                                                             shared_crypts.clone())
                                   })
